...
//import { donationBook } from '../../interfaces/BookProduct';
...
@Component({
  selector: 'app-donationbook',
  templateUrl: './donationbook.page.html',
  styleUrls: ['./donationbook.page.scss'],
})
export class DonationbookPage implements OnInit {
  ...
  public donationBook: Publication;
  ...
  getPublications(page, adding = false, city, searchQuery) {
    this.publicationService.getDonationBooks(page, this.city, this.searchQuery).subscribe(
      (res: any) => {
        ...
        if (!adding) {
          ...
          let arrayA = this.donationBooks;
          let arrayB = res['publications'];
          this.donationBooks = arrayA.concat(arrayB);
        }
        if (page > this.pages) {
        }
      },
  ...
  noMore = false;
  viewMore() {
    if (this.donationBooks.length == this.total) {
      this.noMore = true;
    } else {
      this.page += 1;
    }
  ...
  //   const dialogRef = this.dialog.open(DeleteDonationbook);
...
// @Component({
//   selector: 'delete',
//   templateUrl: 'deletedonationbook.component.html',
//   styleUrls: ['./donationbook.component.scss']
// })
// export class DeleteDonationbook {
